...
import {Case, ListOfCases, Coordinate} from "@/types/supabase-global"
...
class MapService{
    ...
    async getNearbyCases(currentlat: number, currentlong: number, distance: number) : Promise<ListOfCases>{
        distance = distance*1000
        const{data: cases, error} = await supabase
        .rpc('find_nearby_cases', {
          currentlat, 
          currentlong, 
          distance
        });
